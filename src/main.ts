declare var confetti: any;
      ...
      const puzzleContainer = document.querySelector(
        "#puzzle-container"
      ) as HTMLDivElement;
      const progress = document.querySelector("#progress") as HTMLDivElement | null;
      const winnerMessage = document.querySelector(
        "#winner-message"
      ...
      async function startGame() {
        ...
        // Extract file name (without extension) as the current logo name.
      ...
      function trackHandPosition(results: GestureRecognizerResult) {
        if (!results?.landmarks || results.landmarks.length === 0 || !selectedPiece)
          return;
